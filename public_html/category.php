					 ...
					 <?php 
				        if(isset($_GET['search'])){
				          $search = $_GET['search'];
				        	...
				        	$sql_seclect = "SELECT * FROM `blog_article` LEFT JOIN category ON blog_article.category_id=category.id WHERE category.cat_slug='$cat' and blog_article.status=1 ORDER BY articleid DESC LIMIT $result1, $post_pre_page";
				           //$sql_seclect = "SELECT * FROM blog_article WHERE status=1 AND category_id='$cat_id' ORDER BY articleid DESC LIMIT $result1, $post_pre_page";
				        }
